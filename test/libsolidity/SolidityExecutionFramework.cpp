...
#include "libsolidity/util/compiler/Compiler.h"
...
)
{
	...
	m_compilerInput = CompilerInput{
		...
		.metadataFormat = !m_appendCBORMetadata ?
			std::make_optional(MetadataFormat::NoMetadata) :
			std::nullopt,
	...
	auto const [sourceName, contractName, isUnqualified] = ContractName::decompose(_contractName);
